...
2. groups.py
3. market_groups.py
4. eve_types.py or tasks.py (both files contain the same code, but tasks.py uses Celery and Redis to allow concurrent data fetching, which drastically reduces the time needed to fetch all data. However, it requires additional setup of Celery and Redis.)
